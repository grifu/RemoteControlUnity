//
//	  Remote Control Interface for Unity based on UnityOSC
//
//	  (Remote Control Window Debuger)
//
...
public class RemoteControl : EditorWindow
{
	...
	/// </summary>
	[MenuItem("Window/RemoteControl")]
	static void Init ()
	{
		RemoteControl window = (RemoteControl)EditorWindow.GetWindow (typeof(RemoteControl));
		window.Show();
	}
	...
	{	
		if(EditorApplication.isPlaying)
		{
			...
			_clients = OSCManager.Instance.Clients;//Get the clients
			_servers = OSCManager.Instance.Servers;//Get the servers
	...
	void Update()
	{
		if(EditorApplication.isPlaying)
		{
			// update only if there is no initializer on the scene
			if(FindObjectsOfType (typeof(RCInit)).Length == 0) OSCManager.Instance.UpdateLogs();
